...
def setup_and_teardown():
    """Setup: Create necessary Person & Course before testing and clean up after tests."""
    ...
    # Ensure test student exists before feedback tests
...
def test_enroll_student():
    ...
    if response.status_code == 201:  # Student was successfully enrolled
        assert "enrolled" in response.json().get("message", "")
    
    elif response.status_code == 409:  # Student is already enrolled
